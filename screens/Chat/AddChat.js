...
export function Item ({ title}){
    ...
    const getUser = async() => {
     await db.collection('doctors').doc(user.email).collection('treats').doc().get()
...
export function renderItem({ item }){
    ...
    const startChat = async() => {
      console.log(item)
        const chatName = (''+user.email+'&&'+item.email).toLocaleLowerCase(); 
          ...
          if (documentSnapchot.exists){
            alert("Ce chat existe déjà");
              ...
              db.collection('chats').doc(chatName).set({
                  chatName:chatName,
                  doctor: user.email,
                  patient:item.email.toLocaleLowerCase(),
              })
              .then(()=>{
    ...
    return(
        <TouchableOpacity style={styles.item} onPress={startChat}>
            ...
            size="medium"
            rounded
            source={require('../../assets/user-710.png')}
            />
            }
            </View>
...
export function AddChat({navigation}){
